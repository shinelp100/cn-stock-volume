...
**已移除缓存机制**，每次运行都会获取最新数据。

- **原因**: 确保数据始终是最新的，避免缓存导致数据过期
- **影响**: 每次运行会调用 browser 工具，耗时略长（约 5-10 秒）
- **优势**: 数据实时性 100%
