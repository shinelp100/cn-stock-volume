...
# 用于查询涨跌家数的市场代码（东方财富市场代码）
EM_MARKET_CODES = {
    "沪市": "m:1 t:2",      # 沪市 A 股
    "深市": "m:0 t:2",      # 深市 A 股（包含创业板）
    "北交所": "m:0 t:81",   # 北交所股票
}
...
# 东方财富涨跌家数 API
EMC_ADVANCE_DECLINE_URL = "https://push2.eastmoney.com/api/qt/clist/get"
...
def query_market_advance_decline(target_date):
    """
    查询三市（沪市/深市/北交所）的上涨/下跌家数
    返回：{ market_name: { status, date, up, down, unchanged, total } }
    """
    results = {}
    
    for market_name, em_code in EM_MARKET_CODES.items():
        try:
            # 东方财富涨跌家数 API 参数
            params = {
                "pn": "1",
            ...
            url = EMC_ADVANCE_DECLINE_URL + "?" + urllib.parse.urlencode(params)
            req = urllib.request.Request(url, headers=HEADERS)
            ...
            # 从 f152 字段获取涨跌家数统计（格式：上涨家数 -下跌家数 -平盘家数）
            advance_decline_str = result_data.get("f152", "")
            
            if advance_decline_str:
                parts = advance_decline_str.split("-")
                if len(parts) == 3:
                    up = int(parts[0])
                    down = int(parts[1])
                    ...
                    results[market_name] = {
                        "status": "parse_error",
                        "message": f"涨跌家数格式异常：{advance_decline_str}",
                    }
            else:
                results[market_name] = {
                    "status": "no_data",
...
def build_advance_decline_summary(advance_decline_results):
    ...
    汇总三市涨跌家数数据
    """
    ok_markets = {k: v for k, v in advance_decline_results.items() if v["status"] == "ok"}
    
    if not ok_markets:
        return None
    
    total_up = sum(v["up"] for v in ok_markets.values())
    total_down = sum(v["down"] for v in ok_markets.values())
    total_unchanged = sum(v["unchanged"] for v in ok_markets.values())
    grand_total = total_up + total_down + total_unchanged
    ...
    return {
        ...
        "up_ratio": round(total_up / grand_total * 100, 2) if grand_total > 0 else None,
        "down_ratio": round(total_down / grand_total * 100, 2) if grand_total > 0 else None,
        "market_details": ok_markets,
    }
